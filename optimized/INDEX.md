...
| DeepSeek Harness 标志（v0.9.17） | `09_background.js` + `07_css_builder.js` | `BrandEditor`（侧边栏子版块、新会话之上：标志颜色 + 透明度，默认品牌蓝 #3964fe，与正文解耦）；CSS = `buildBrandCss`（`[class*="_logoRow"] [class*="_brand"] svg`，BrandWordmark path fill=currentColor → 设 color 即变色）；状态字段 `brand`；**v1.0.5 收起态**：`brand.collapsed`（null=跟随展开，折叠后标志 = `hHd-Xa_railFish` 鲸鱼图标，`[class*="_collapsed"] [class*="_logoRow"] [class*="_railFish"]`） |
...
| 新会话欢迎页四项调色（v1.0.5） | `09_background.js` + `07_css_builder.js` | 点「新会话」后的空态页（HeroShell）：鲸鱼 `_fishHitbox` svg / 探索未至之境 `_headlineText` / 预览版文字 `_previewBadge` / 预览版背景；状态 `hero = { fish, title, badge, badgeBg }` 各 `{ color, opacity }`（null=官方）；CSS = `buildHeroCss`（⚠️ 未设置时注入 **static 层官方原色**（亮 `--dsw-static-neutral-bluish-1000` 等 / 暗 `body[data-ds-dark-theme]`）→ 与正文文字颜色解耦——label-primary 会被文字颜色 overrideTokens 覆盖）；`<style data-thmcz-hero>` **无条件渲染**（条件渲染 = 默认空 CSS 时第一次拖动不实时） |
